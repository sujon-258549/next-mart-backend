...
const getAllBrand = async (query: Record<string, unknown>) => {
  const brandQuery = new QueryBuilder(Brand.find().populate("createdBy"), query)
    .search(["name"])
    .filter()
    .sort()
    .paginate()
...
const deleteBrandIntoDB = async (id: string, authUser: IJwtPayload) => {
  const isBrandExist = await Brand.findById(id);
  if (!isBrandExist) {
    throw new AppError(StatusCodes.NOT_FOUND, "Brand not found!");
  }
  ...
  if (product)
    throw new AppError(
      StatusCodes.BAD_REQUEST,
      "You can not delete the brand. Because the brand is related to products."
    );
